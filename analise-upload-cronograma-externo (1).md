...
#### Estrutura real identificada no cronograma do CESMAC

Após análise de um cronograma real enviado por professor, foram identificados os seguintes padrões e desafios:

```
Estrutura do documento real:
┌─────────────────────────────────────────────────────────────┐
│ CRONOGRAMA DIDÁTICO DO PROCESSO FORMATIVO  (título/cabeçalho│
│ 1. CURSO: BIOMEDICINA                                        │
│ 2. COMPONENTE CURRICULAR: CITOPATOLOGIA  CH: 60h            │
│ 3. SEMESTRE: 26/02  Período: 5º  Turno: MATUTINO            │
│ 4. COORDENADORA DO CURSO: CHIARA RACHEL...                   │
│ 5. DOCENTE(S): ROMUALDO ARTHUR...                            │
├──────────┬──────────┬──────────┬───┬──────────┬─────────────┤
│ ENCONTRO │  DATA    │ HORÁRIO  │CH │  TEMA    │  DOCENTE(S) │
├──────────┼──────────┼──────────┼───┼──────────┼─────────────┤
│  1ª ETAPA (linha de separação — sem dados de aula)           │
├──────────┼──────────┼──────────┼───┼──────────┼─────────────┤
│    1     │ 31/07    │07h30 às  │ 3 │Apresent. │  ROMUALDO   │
│          │ Sexta    │09h10     │   │da discipl│             │
│          │          │09h30 às  │   │Introdução│             │
│          │          │10h20     │   │ao estudo │             │
├──────────┼──────────┼──────────┼───┼──────────┼─────────────┤
│    2     │ 07/08    │  ...     │...│  ...     │  Romualdo   │
│  ...     │  ...     │  ...     │...│  ...     │  ...        │
│  2ª ETAPA (nova linha de separação)                          │
│  ...     │  ...     │  ...     │...│  ...     │  ...        │
│  3ª ETAPA (nova linha de separação)                          │
│  ...     │  ...     │  ...     │...│  ...     │  ...        │
│  FINALIZAÇÃO DO PROCESSO (linha de rodapé — ignorar)         │
└──────────┴──────────┴──────────┴───┴──────────┴─────────────┘
```

**Desafios identificados no formato real:**

| Desafio | Exemplo real | Solução no parser |
|---|---|---|
| Tabela com 16 colunas mescladas | `colspan="4"` no tema | Ignorar estrutura de colspan; usar posição textual |
| Cabeçalho em múltiplas linhas mescladas | "ENCONTRO", "DATA", "HORÁRIO" em rowspan=2 | Identificar colunas por palavras-chave, não por índice fixo |
| Datas sem ano | `31/07`, `07/08` | Inferir ano pelo semestre lido no cabeçalho do doc |
| Horário em dois blocos na mesma célula | `07h30 às 09h10` + `09h30 às 10h20` | Extrair todos os intervalos; usar o primeiro como início e o último como fim |
| Formato de hora com "h" | `07h30`, `10h20` | Normalizar: `07h30` → `07:30` |
| Linhas divisórias entre etapas | `1ª ETAPA`, `2ª ETAPA` | Detectar e ignorar linhas onde a célula de data está vazia |
| Local na coluna "PRESENCIAL" | `Laboratório e sala de aula` | Extrair como campo `local`; mapear para lab do sistema na análise |
| Linha de FINALIZAÇÃO | Totais e assinaturas | Ignorar linhas sem número de encontro válido |
| Metadados no cabeçalho | Curso, CH, Docente, Coordenadora | Extrair como contexto global do arquivo (aplicar a todos os itens) |

#### Estratégia de parser para o formato CESMAC
...
interface ContextoDocumento {
  curso: string;
  componente: string;       // disciplina/matéria
  cargaHoraria: string;
  semestre: string;
  turno: string;
  docente: string;
  coordenadora: string;
  anoInferido: number;      // extraído da data do semestre (ex: "26/02" → ano atual)
}

async function parseDocx(arquivo: File): Promise<{ contexto: ContextoDocumento; itens: ItemBruto[] }> {
  const arrayBuffer = await arquivo.arrayBuffer();
  const { value: html } = await mammoth.convertToHtml({ arrayBuffer });

  const dom = new DOMParser().parseFromString(html, 'text/html');
  const tabela = dom.querySelector('table');

  if (!tabela) {
    return { contexto: {} as ContextoDocumento, itens: [] };
  }

  const linhas = Array.from(tabela.querySelectorAll('tr'));

  // PASSO 1 — Extrair metadados do cabeçalho (primeiras ~5 linhas)
  const contexto = extrairContexto(linhas);

  // PASSO 2 — Localizar a linha de cabeçalho da tabela de encontros
  // (linha que contém "ENCONTRO" e "DATA" e "HORÁRIO")
  const idxCabecalho = linhas.findIndex(tr =>
    tr.textContent?.toUpperCase().includes('ENCONTRO') &&
    tr.textContent?.toUpperCase().includes('DATA')
  );

  if (idxCabecalho === -1) return { contexto, itens: [] };

  // PASSO 3 — Processar linhas de dados (após o cabeçalho)
  ...
  for (const linha of linhas.slice(idxCabecalho + 1)) {
    ...
    // Ignorar linhas de separação de etapa (ex: "1ª ETAPA", "2ª ETAPA")
    if (/^\d+ª\s+ETAPA/i.test(texto) || /FINALIZAÇÃO/i.test(texto)) continue;

    // Ignorar linhas de rodapé sem número de encontro
    const numEncontro = parseInt(cels[0]?.textContent?.trim() ?? '');
    if (isNaN(numEncontro)) continue;

    // Extrair data (coluna 1) — formato "31/07\nSexta"
    const textoData = cels[1]?.textContent?.trim() ?? '';
    const dataMatch = textoData.match(/(\d{1,2})\/(\d{1,2})/);
    const data = dataMatch
      ? `${dataMatch[1]}/${dataMatch[2]}/${contexto.anoInferido}`
      : null;

    // Extrair horários — pode haver dois blocos: "07h30 às 09h10\n09h30 às 10h20"
    const textoHorario = cels[2]?.textContent?.trim() ?? '';
    const horarios = extrairHorarios(textoHorario);

    // Extrair tema (coluna 4 — mas com colspan variável, usar posição 4+)
    const tema = cels[4]?.textContent?.trim()
      ?? cels[3]?.textContent?.trim()
      ?? '';

    // Extrair local (coluna "PRESENCIAL") — posição ~10-13 dependendo de colspan
    const localRaw = extrairLocal(cels);

    itens.push({
      encontro:      numEncontro,
      data,
      horarioInicio: horarios.inicio,
      horarioFim:    horarios.fim,
      tema,
      local:         localRaw,               // "Laboratório e sala de aula"
      laboratorio:   inferirLaboratorio(localRaw), // mapeamento para lab do sistema
      disciplina:    contexto.componente,
      professor:     cels[cels.length - 1]?.textContent?.trim() ?? contexto.docente,
      curso:         contexto.curso,
      turno:         contexto.turno,
      cargaHoraria:  parseInt(cels[3]?.textContent?.trim() ?? '0') || 3,
    });
  }
...
// Normaliza "07h30 às 09h10\n09h30 às 10h20" → { inicio: "07:30", fim: "10:20" }
function extrairHorarios(texto: string): { inicio: string; fim: string } {
  // Substituir "h" por ":" e extrair todos os horários
  const normalizado = texto.replace(/(\d{1,2})h(\d{2})/g, '$1:$2');
  const horas = normalizado.match(/\d{1,2}:\d{2}/g) ?? [];

  return {
    inicio: horas[0] ?? '',
    fim:    horas[horas.length - 1] ?? '',  // último horário = fim do último bloco
  };
}

// Extrai metadados do cabeçalho do documento
function extrairContexto(linhas: Element[]): ContextoDocumento {
  const texto = linhas.slice(0, 8).map(l => l.textContent ?? '').join('\n');

  const curso       = extrairApos(texto, 'CURSO');
  const componente  = extrairApos(texto, 'COMPONENTE CURRICULAR');
  const ch          = texto.match(/CH[:\s]+(\d+h?)/i)?.[1] ?? '';
  const turno       = extrairApos(texto, 'TURNO');
  const docente     = extrairApos(texto, 'DOCENTE');
  const coordenadora= extrairApos(texto, 'COORDENADORA');

  // Inferir ano pelo semestre ("26/02" ou "2025.1")
  const anoMatch = texto.match(/\b(202\d)\b/);
  const anoInferido = anoMatch ? parseInt(anoMatch[1]) : new Date().getFullYear();

  return { curso, componente, cargaHoraria: ch, semestre: '', turno, docente, coordenadora, anoInferido };
}

// Mapeia texto livre do local para laboratório do sistema
// Esta função deve ser alimentada com os labs cadastrados no Firestore
function inferirLaboratorio(localRaw: string): string | null {
  const l = localRaw.toLowerCase();
  if (l.includes('laboratório') || l.includes('lab')) {
    // Retorna null para indicar que o coordenador deve selecionar o lab
    // na tela de revisão (campo obrigatório que não pode ser inferido)
    return null;
  }
  // "Sala de aula" → não é um agendamento de laboratório
  // O sistema pode sugerir ignorar este item ou marcar como "Sala de aula"
  return 'sala_de_aula';
}
```

#### Lógica de seleção inteligente na tela de revisão

Como o documento real mistura **aulas em sala** com **aulas em laboratório**, o sistema deve tratar isso na tela de revisão:

```
🟡 Encontro 3 — 14/08 · "Coleta, Coloração e Fixação. Aula prática de microscopia"
   Local informado: "Laboratório e sala de aula"
   ⚠ Laboratório não identificado automaticamente — selecione o laboratório:
   [ Anatomia 1 ▼ ]    ← dropdown com labs do sistema

🔘 Encontro 1 — 31/07 · "Apresentação da disciplina"
   Local informado: "Sala de aula"
   ℹ Este encontro é em sala de aula, não em laboratório.
   [Incluir mesmo assim] [Ignorar]
```

#### Aviso sobre `.doc` (formato antigo binário)

`.doc` (Word 97-2003) **não pode ser lido diretamente no browser**. O sistema exibe aviso claro:

```jsx
if (arquivo.name.endsWith('.doc') && !arquivo.name.endsWith('.docx')) {
  return (
    <Alert severity="warning">
      <AlertTitle>Formato .doc não suportado</AlertTitle>
      Abra no Word e salve como <strong>.docx</strong>:
      Arquivo → Salvar como → Word (.docx)
    </Alert>
  );
}
```
